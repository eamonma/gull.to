...
describe('Domain Types - TDD RED Phase', () => {
  describe('Alpha4Code branded type', () => {
    ...
    it('should reject invalid formats', () => {
      expect(() => createAlpha4Code('amcr')).toThrow('Invalid Alpha4Code format');
      expect(() => createAlpha4Code('TOOLONG')).toThrow('Invalid Alpha4Code format');
      expect(() => createAlpha4Code('AM1R')).toThrow('Invalid Alpha4Code format');
  ...
  describe('EBirdCode branded type', () => {
    ...
    it('should reject invalid formats', () => {
      expect(() => createEBirdCode('AMECRO')).toThrow('Invalid EBirdCode format');  // uppercase
      expect(() => createEBirdCode('toolongname')).toThrow('Invalid EBirdCode format');  // too long
      ...
      expect(() => createEBirdCode('ame-ro')).toThrow('Invalid EBirdCode format'); // invalid character
    });
  ...
  describe('ScientificName branded type', () => {
    ...
    it('should reject invalid formats', () => {
      expect(() => createScientificName('corvus americanus')).toThrow('Invalid ScientificName format');
      expect(() => createScientificName('Corvus')).toThrow('Invalid ScientificName format');
      expect(() => createScientificName('')).toThrow('Invalid ScientificName format');
    });
  ...
  describe('Version types for traceability', () => {
    ...
    it('should enforce CalVer for MapVersion per instructions.md:196', () => {
      ...
      expect(() => ({ version: '2024.09-hotfix.1' }) as MapVersion).not.toThrow();
    });
  });
